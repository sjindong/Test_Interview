package com.test_interview.AliyunMQTT.shadow;
...
import com.test_interview.AliyunMQTT.ALiyunIotX509TrustManager;
import com.test_interview.AliyunMQTT.Contain;
import com.test_interview.util.AliyunWebUtils;
import com.test_interview.util.LogUtil;
...
public class SimpleClient4Shadow {
     ...
     * 认证服务器地址 每个区域不一样
     */
    private static String authUrl = "https://iot-auth.cn-shanghai.aliyuncs.com/auth/" + Contain.deviceName;

    /**
     * 设备key和secret信息
     */
    private static String deviceName = Contain.deviceName;
    private static String productKey = Contain.ProductKey;
    private static String deviceSecret = Contain.deviceSecret;
    ...
    public SimpleClient4Shadow(Context context){
        ...
        String result = null;
        try {
            result = AliyunWebUtils.doPost(authUrl, params, 5000, 5000);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        saveToken(context, "iotId", iotId);
        saveToken(context, "iotToken", token);
        saveToken(context, "targetServer", targetServer);
        Log.e("SJD","iotId = "+iotId);
        Log.e("SJD","iotToken = "+token);
        Log.e("SJD","targetServer = "+targetServer);
    ...
    private static SSLSocketFactory createSSLSocket(Context context1) throws Exception {
        SSLContext context = SSLContext.getInstance("TLSV1.2");
        context.init(null, new TrustManager[]{new ALiyunIotX509TrustManager(context1)}, null);
