...
public class SimpleClient4IOT {
    Context context;
    /******这里是客户端需要的参数*******/
    public static String deviceName = Contain.deviceName;
    public static String productKey = Contain.ProductKey;
    public static String secret = Contain.deviceSecret;

    //用于测试的topic
    private static String subTopic = "/" + productKey + "/" + deviceName + "/get";
    private static String pubTopic = "/" + productKey + "/" + deviceName + "/update";

    public SimpleClient4IOT(Context context) {
    ...
    public void connectMqtt(String url, String clientId, String mqttUsername,
        ...
        sampleClient.setCallback(new MqttCallback() {
            ...
            public void deliveryComplete(IMqttDeliveryToken token) {
                ...
                LogUtil.print(""+token.getResponse());
     ...
     *      
     */
    public void sendMessage( String message) throws IllegalArgumentException, UnsupportedEncodingException {
        try {

            if (sampleClient == null || !sampleClient.isConnected()) {
                sampleClient.connect();
            }
            Log.e("SJD", "send message to "  + ", message is " + message);
            // 发布自己的消息  
            sampleClient.publish(subTopic, message.getBytes("utf-8"), 1, true);
        } catch (MqttException e) {
            Log.e("SJD", String.valueOf(e.getCause()));
            e.printStackTrace();
        }
    }
